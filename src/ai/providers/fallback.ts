...
export async function generateWithFallback(
  ...
  if (isCircuitOpen()) {
    throw new Error(
      '🔴 Fallback provider circuit breaker is open. Service temporarily unavailable.'
    );
  }
  ...
  try {
    ...
    throw new Error(
      `⚠️ Fallback provider failed: ${error.message}. Retrying with another provider.`
    );
  }
}
 ...
 */
function buildFallbackPrompt(request: AIRequest): string {
...
function recordFailure(): void {
  ...
  if (circuitBreaker.failureCount >= FAILURE_THRESHOLD) {
    ...
    console.warn(
      `🔴 Fallback provider circuit breaker opened after ${FAILURE_THRESHOLD} failures`
    );
  }
}
