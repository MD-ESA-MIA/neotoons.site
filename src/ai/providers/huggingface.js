...
export async function generateImagePromptWithHuggingFace(userInput, modelQuality = 'quality') {
 ...
 */
export async function generateImage(imagePrompt, modelQuality = 'quality') {
    if (!HUGGINGFACE_API_KEY) {
        throw new Error('🔑 HUGGINGFACE_API_KEY is not configured');
    }
    try {
        const modelId = IMAGE_GENERATION_MODELS[modelQuality];
        const response = await fetch(`${HUGGINGFACE_BASE_URL}/models/${modelId}`, {
            headers: {
...
async function generateImagePrompt(userInput) {
    ...
    const finalPrompt = [
        ...
        .join(', ')
        .slice(0, 480);
    const negativePrompt = 'blurry, low quality, distorted, bad anatomy, extra limbs, noisy, oversaturated, watermark, text artifacts';
    return `Final Prompt:\n"${finalPrompt}"\n\nNegative Prompt:\n"${negativePrompt}"`;
}
...
function detectEnvironment(lowerInput) {
...
function detectStyle(lowerInput) {
    if (lowerInput.includes('anime'))
        return 'anime style with crisp linework and expressive color grading';
    if (lowerInput.includes('cinematic'))
        return 'cinematic visual style with filmic color grading';
    if (lowerInput.includes('digital art'))
        return 'digital art style with polished rendering';
    if (lowerInput.includes('realistic') || lowerInput.includes('photoreal')) {
        return 'photorealistic style with lifelike textures';
    }
    if (lowerInput.includes('logo'))
        return 'minimal, clean vector-inspired style';
    if (lowerInput.includes('portrait'))
        return 'realistic portrait style with natural skin detail';
    if (lowerInput.includes('fantasy'))
        return 'cinematic fantasy art style with dramatic atmosphere';
    if (lowerInput.includes('product'))
        return 'premium product photography style';
    return 'realistic cinematic style';
}
function detectLighting(lowerInput, style) {
    if (lowerInput.includes('studio') || lowerInput.includes('product')) {
...
function detectCameraAngle(lowerInput) {
...
function buildDetails(lowerInput) {
    const details = [];
    if (lowerInput.includes('logo')) {
...
export async function testHuggingFaceConnection() {
    try {
        const response = await fetch(`${HUGGINGFACE_BASE_URL}/api/models?search=stable-diffusion`, {
            headers: {
                Authorization: `Bearer ${HUGGINGFACE_API_KEY}`,
            },
        });
