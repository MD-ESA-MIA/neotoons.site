...
function splitTextIntoChunks(text, maxLength) {
    ...
    while (remaining.length > maxLength) {
        const punctuationSplit = Math.max(remaining.lastIndexOf('. ', maxLength), remaining.lastIndexOf('! ', maxLength), remaining.lastIndexOf('? ', maxLength), remaining.lastIndexOf(', ', maxLength));
...
function normalizeSpeed(inputSpeed) {
    if (typeof inputSpeed !== 'number' || Number.isNaN(inputSpeed))
        return 1;
    return Math.max(0.5, Math.min(1.5, inputSpeed));
}
export function mapSpeedToSlow(speed) {
    const normalized = normalizeSpeed(speed);
    return normalized < 0.85;
}
export async function* generateVoiceAudioChunks(text, options = {}) {
    const lang = options.lang || 'en';
    const slow = typeof options.slow === 'boolean' ? options.slow : mapSpeedToSlow(options.speed);
...
export async function generateVoiceAudioBuffer(text, options = {}) {
    const audioBuffers = [];
